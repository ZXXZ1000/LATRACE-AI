...
LATRACE 是一个**生产级多模态记忆服务**，面向聊天助手、Copilot、陪伴型 Agent 与各类智能体系统，提供真正可控的长期记忆能力。

它不是简单地把更多文本塞进上下文窗口，而是把对话、音频、图像和视频转化为一层可查询、可追踪、可解释的活体记忆：覆盖人物、偏好、事实、关系、状态与时间线。

更重要的是，这些模态不会形成彼此割裂的记忆孤岛，而是会汇聚到同一张带时间语义的图谱里。这样一来，一句对白、一段音频、一个视频片段都能用同一条证据链解释。
...
- 让文本、音频、图片、视频进入同一张图谱，并共享同一套检索与解释面
...
- 文本通过 `POST /ingest/dialog/v1` 进入
- 音频通过 `POST /ingest/media/audio/v1` 进入
- 视频通过 `POST /ingest/media/video/v1` 进入
- 最终都落到同一张 TKG 图里，共享 `tenant_id`、`user_id`、`memory_domain`、时间语义、写入和检索契约
...
- 如果你要接入多模态记忆，可以直接调用 `POST /ingest/media/video/v1` 和 `POST /ingest/media/audio/v1`
...
不管选哪种模式，所有模态最后都会收敛到同一张图、同一套隔离和同一套可解释输出。
...
[Star 我们的项目](https://github.com/ZXXZ1000/LATRACE-AI) · [API 文档](docs/api_reference.md) · [Benchmark 指南](docs/benchmark_guide.md) · [Discussions](https://github.com/ZXXZ1000/LATRACE-AI/discussions)
